...
class ModelMenuMenu extends Model {
    ...
    public function getNameMenuModule($menu_id, $menu_type_id, $name_id) {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "menu mg LEFT JOIN " . DB_PREFIX . "menu_description mgd ON(mg.menu_id = mgd.menu_id) WHERE mgd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND mg.menu_id = '" . $menu_id . "'");
